...
export default function Tasks() {
  const { tasks, toggleTask, addTask } = useAppContext();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={handleAddTask} className="flex items-center gap-2 px-4 py-2 bg-black text-white rounded-lg font-medium text-sm hover:bg-gray-800 transition-colors shadow-md">
                <Plus size={18} />
        ...
        <aside className="w-full md:w-64 flex-shrink-0">
          <nav className="space-y-1">
            ...
            >
              ...
              <span className="bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs font-bold">{tasks.length}</span>
            </button>
            <button
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            <div className="space-y-1">
              {tasks.map(task => (
                <div key={task.id} className={`group flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100 ${task.completed ? 'opacity-60' : ''}`}>
                  <div className="mt-1 flex-shrink-0" onClick={() => toggleTask(task.id)}>
                    <div className={`w-5 h-5 rounded border-2 ${task.completed ? 'border-black bg-black text-white flex items-center justify-center' : 'border-gray-300 hover:border-black'} cursor-pointer transition-colors`}>
                      {task.completed && <CheckCircle size={14} />}
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-4 mb-1">
                      <h4 className={`text-base font-semibold truncate ${task.completed ? 'text-gray-500 line-through' : 'text-gray-900'}`}>{task.title}</h4>
